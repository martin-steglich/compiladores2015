...
public class BinaryOperationExpression extends Expression {
	...
	public BinaryOperationExpression(Expression lop, String type,
			Expression rop, int lineNumber) {
		left = lop;
		right = rop;
		op = type;
	...
	private Type suma(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			int resultado = ((IntegerType) izq).getValue()
					+ ((IntegerType) der).getValue();
			res = new IntegerType(resultado);
		} else if (intLong) {
			long resultado = ((IntegerType) izq).getValue()
					+ ((LongType) der).getValue();
			res = new LongType(resultado);
		} else if (intFloat) {
			float resultado = ((IntegerType) izq).getValue()
					+ ((FloatType) der).getValue();
			res = new FloatType(resultado);
		} else if (intBoolean) {
			int resultado = ((IntegerType) izq).getValue()
					+ ((BooleanType) der).getEquivalentInt();
			res = new IntegerType(resultado);
		} else if (stringString) {
			String resultado = ((StringType) izq).getText()
					+ ((StringType) der).getText();
			res = new StringType(resultado);
		} else if (longInt) {
			long resultado = ((LongType) izq).getValue()
					+ ((IntegerType) der).getValue();
			res = new LongType(resultado);
		} else if (longLong) {
			long resultado = ((LongType) izq).getValue()
			...
			float resultado = ((LongType) izq).getValue()
					+ ((FloatType) der).getValue();
			res = new FloatType(resultado);
		} else if (longBoolean) {
			long resultado = ((LongType) izq).getValue()
			...
			float resultado = ((FloatType) izq).getValue()
					+ ((IntegerType) der).getValue();
			res = new FloatType(resultado);
		} else if (floatLong) {
			float resultado = ((FloatType) izq).getValue()
			...
			int resultado = ((BooleanType) izq).getEquivalentInt()
					+ ((IntegerType) der).getValue();
			res = new IntegerType(resultado);
		} else if (booleanLong) {
			long resultado = ((BooleanType) izq).getEquivalentInt()
					+ ((LongType) der).getValue();
			res = new LongType(resultado);
		} else if (booleanFloat) {
			float resultado = ((BooleanType) izq).getEquivalentInt()
					+ ((FloatType) der).getValue();
			res = new FloatType(resultado);
			...
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion suma");
		}
	...
	private Type resta(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			...
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion resta");
		}
	...
	private Type multiplicacion(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			...
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion multiplicacion");
		}
	...
	private Type division(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			...
			if (div == 0) {
				throw new CompilerException(lineNumber,
						"No es posible realizar una division por cero");
			}
			...
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion division");
		}
	...
	private Type divisionEntera(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			...
			float resultado = (float) Math.floor(((IntegerType) izq).getValue()
					/ div);
			res = new FloatType(resultado);
		} else if (intBoolean) {
			int div = ((BooleanType) der).getEquivalentInt();
			...
			if (div == 0f)
				throw new CompilerException(lineNumber,
						"No es posible realizar una division por cero");
			float resultado = (float) Math.floor(((LongType) izq).getValue()
					/ div);
			res = new FloatType(resultado);
		} else if (longBoolean) {
			int div = ((BooleanType) der).getEquivalentInt();
			...
			float resultado = (float) Math.floor(((FloatType) izq).getValue()
					/ div);
			res = new FloatType(resultado);
		} else if (floatLong) {
			long div = ((LongType) der).getValue();
			...
			float resultado = (float) Math.floor(((BooleanType) izq)
					.getEquivalentInt() / div);
			res = new FloatType(resultado);
		} else if (booleanBoolean) {
			int div = ((BooleanType) der).getEquivalentInt();
			if (div == 0)
	...
	private Type and(Type izq, Type der) throws CompilerException {
		...
		boolean tupleDict = (izq.getType() == 11) && (der.getType() == 10);
		...
		boolean booleanDict = (izq.getType() == 5) && (der.getType() == 10);
		boolean floatDict = (izq.getType() == 1) && (der.getType() == 10);

		boolean longDict = (izq.getType() == 2) && (der.getType() == 10);

		boolean stringDict = (izq.getType() == 3) && (der.getType() == 10);
		boolean intDict = (izq.getType() == 0) && (der.getType() == 10);
		boolean listDict = (izq.getType() == 6) && (der.getType() == 10);

		boolean dictInt = (izq.getType() == 10) && (der.getType() == 0);
		boolean dictString = (izq.getType() == 10) && (der.getType() == 3);
		boolean dictLong = (izq.getType() == 10) && (der.getType() == 2);
		boolean dictBoolean = (izq.getType() == 10) && (der.getType() == 5);
		boolean dictFloat = (izq.getType() == 10) && (der.getType() == 1);
		boolean dictTuple = (izq.getType() == 10) && (der.getType() == 11);
		boolean dictList = (izq.getType() == 10) && (der.getType() == 6);
		boolean dictDict = (izq.getType() == 10) && (der.getType() == 10);
		...
		if (intInt) {
			...
			int intIzq = ((IntegerType) izq).getValue();
			Map<Type, Type> dictDer = ((DictionaryType) der).getDictionary();
			if (intIzq == 0)
				res = new IntegerType(0);
			else
				res = new DictionaryType(dictDer);
			...
			int intDer = ((IntegerType) der).getValue();
			Map<Type, Type> dictIzq = ((DictionaryType) izq).getDictionary();
			if (dictIzq.isEmpty())
				res = new DictionaryType(dictIzq);
			else
				res = new IntegerType(intDer);
			...
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion and");
		}
	...
	private Type or(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			...
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion or");
		}
	...
	private Type equals(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			boolean resultado = ((IntegerType) izq).getValue() == ((IntegerType) der)
					.getValue();
			res = new BooleanType(resultado);
		} else if (intLong) {
			boolean resultado = ((IntegerType) izq).getValue() == ((LongType) der)
					.getValue();
			res = new BooleanType(resultado);
		} else if (intFloat) {
			boolean resultado = ((IntegerType) izq).getValue() == ((FloatType) der)
					.getValue();
			res = new BooleanType(resultado);
		} else if (intBoolean) {
			boolean resultado = ((IntegerType) izq).getValue() == ((BooleanType) der)
					.getEquivalentInt();
			res = new BooleanType(resultado);
		} else if (intString) {
			res = new BooleanType(false);
		} else if ((intTuple) || (intList) || (intDict)) {
			res = new BooleanType(false);
		} else if ((stringInt) || (stringBoolean) || (stringLong)
				|| (stringFloat) || (stringTuple) || (stringList)
				|| (stringDict)) {
			res = new BooleanType(false);
		} else if (stringString) {
			...
			boolean resultado = ((LongType) izq).getValue() == ((IntegerType) der)
					.getValue();
			res = new BooleanType(resultado);
		} else if (longLong) {
			boolean resultado = ((LongType) izq).getValue() == ((LongType) der)
					.getValue();
			res = new BooleanType(resultado);
		} else if (longFloat) {
			boolean resultado = ((LongType) izq).getValue() == ((FloatType) der)
					.getValue();
			res = new BooleanType(resultado);
		} else if (longBoolean) {
			boolean resultado = ((LongType) izq).getValue() == ((BooleanType) der)
					.getEquivalentInt();
			res = new BooleanType(resultado);
		} else if (longString) {
			res = new BooleanType(false);
		} else if ((longTuple) || (longList) || (longDict)) {
			res = new BooleanType(false);
		} else if (floatInt) {
			boolean resultado = ((FloatType) izq).getValue() == ((IntegerType) der)
					.getValue();
			res = new BooleanType(resultado);
		} else if (floatLong) {
			boolean resultado = ((FloatType) izq).getValue() == ((LongType) der)
					.getValue();
			res = new BooleanType(resultado);
		} else if (floatFloat) {
			boolean resultado = ((FloatType) izq).getValue() == ((FloatType) der)
					.getValue();
			res = new BooleanType(resultado);
		} else if (floatBoolean) {
			boolean resultado = ((FloatType) izq).getValue() == ((BooleanType) der)
					.getEquivalentInt();
			res = new BooleanType(resultado);
		} else if (floatString) {
			res = new BooleanType(false);
		} else if ((floatTuple) || (floatList) || (floatDict)) {
			res = new BooleanType(false);
		} else if (booleanInt) {
			boolean resultado = ((BooleanType) izq).getEquivalentInt() == ((IntegerType) der)
					.getValue();
			res = new BooleanType(resultado);
		} else if (booleanLong) {
			boolean resultado = ((BooleanType) izq).getEquivalentInt() == ((LongType) der)
					.getValue();
			res = new BooleanType(resultado);
		} else if (booleanFloat) {
			boolean resultado = ((BooleanType) izq).getEquivalentInt() == ((FloatType) der)
					.getValue();
			res = new BooleanType(resultado);
		} else if (booleanBoolean) {
			boolean resultado = ((BooleanType) izq).getValue() == ((BooleanType) der)
					.getValue();
			res = new BooleanType(resultado);

		} else if (booleanString) {
			res = new BooleanType(false);
		} else if ((booleanTuple) || (booleanList) || (booleanDict)) {
			res = new BooleanType(false);
		} else if ((tupleInt) || (tupleBoolean) || (tupleLong) || (tupleFloat)
				|| (tupleString) || (tupleList) || (tupleDict)) {
			res = new BooleanType(false);
		} else if (tupleTuple) {
			ArrayList<Type> tupleIzq = ((TupleType) izq).getTuple();
			ArrayList<Type> tupleDer = ((TupleType) der).getTuple();
			...
			res = new BooleanType(resultado);
		} else if ((listInt) || (listBoolean) || (listLong) || (listFloat)
				|| (listString) || (listTuple) || (listDict)) {
			res = new BooleanType(false);
		} else if (listList) {
			ArrayList<Type> listIzq = ((ListType) izq).getList();
			ArrayList<Type> listDer = ((ListType) der).getList();
			boolean resultado = listIzq.equals(listDer);
			res = new BooleanType(resultado);
		} else if ((dictInt) || (dictBoolean) || (dictLong) || (dictFloat)
				|| (dictString) || (dictTuple) || (dictList)) {
			res = new BooleanType(false);
		} else if (dictDict) {
			Map<Type, Type> dictIzq = ((DictionaryType) izq).getDictionary();
			Map<Type, Type> dictDer = ((DictionaryType) der).getDictionary();
			boolean resultado = dictIzq.equals(dictDer);
			res = new BooleanType(resultado);
		} else {
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion equals");
		}
	...
	private Type notEquals(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			...
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion not equals");
		}
	...
	private Type menor(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			...
			boolean resultado = ((BooleanType) izq).getEquivalentInt() < ((BooleanType) der)
					.getEquivalentInt();
			res = new BooleanType(resultado);
			...
			if (tupleIzq.size() < tupleDer.size())
				...
				for (int i = 0; i < tupleIzq.size(); i++) {
					boolean menorIzq = ((BooleanType) menor(tupleIzq.get(i),
							tupleDer.get(i))).getBooleanValue();
					boolean menorDer = ((BooleanType) menor(tupleDer.get(i),
							tupleIzq.get(i))).getBooleanValue();
					if (menorIzq) {
						resultado = true;
						break;
					} else if (menorDer) {
			...
			ArrayList<Type> listDer = ((ListType) der).getList();
			boolean resultado = false;
			if (listIzq.size() < listDer.size())
				resultado = true;
			else if (listDer.size() < listIzq.size())
				resultado = false;
			else
				for (int i = 0; i < listIzq.size(); i++) {
					boolean menorIzq = ((BooleanType) menor(listIzq.get(i),
							listDer.get(i))).getBooleanValue();
					boolean menorDer = ((BooleanType) menor(listDer.get(i),
							listIzq.get(i))).getBooleanValue();
					if (menorIzq) {
						resultado = true;
						break;
					} else if (menorDer) {
			...
			Map<Type, Type> dictDer = ((DictionaryType) der).getDictionary();
			boolean resultado = false;
			if (dictIzq.size() < dictDer.size())
				resultado = true;
			else if (dictDer.size() < dictIzq.size())
				resultado = false;
			else {
				boolean keyMenorIzq = false;
				boolean keyMenorDer = false;
				for (Type keyIzq : dictIzq.keySet()) {
					for (Type keyDer : dictDer.keySet()) {
						keyMenorIzq = ((BooleanType) menor(keyIzq, keyDer))
								.getBooleanValue();
						keyMenorDer = ((BooleanType) menor(keyDer, keyIzq))
								.getBooleanValue();
						if (keyMenorIzq) {
							resultado = true;
							break;
					...
					if ((keyMenorIzq) || (keyMenorDer)) {
						break;
					}
				}
				if ((!keyMenorIzq) && (!keyMenorDer)) {
					boolean valueMenorIzq = false;
					boolean valueMenorDer = false;
					for (Type valueIzq : dictIzq.values()) {
						for (Type valueDer : dictDer.values()) {
							valueMenorIzq = ((BooleanType) menor(valueIzq,
									valueDer)).getBooleanValue();
							valueMenorDer = ((BooleanType) menor(valueDer,
									valueIzq)).getBooleanValue();
							if (valueMenorIzq) {
								resultado = true;
								break;
							} else if (valueMenorDer) {
								resultado = false;
								break;
							}
						}
						if ((valueMenorIzq) || (valueMenorDer)) {
							break;
						}
					}
				}
			...
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion menor");
		}
	...
	private Type mayor(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			...
			if (tupleIzq.size() > tupleDer.size())
				...
				for (int i = 0; i < tupleIzq.size(); i++) {
					boolean mayorIzq = ((BooleanType) mayor(tupleIzq.get(i),
							tupleDer.get(i))).getBooleanValue();
					boolean mayorDer = ((BooleanType) mayor(tupleDer.get(i),
							tupleIzq.get(i))).getBooleanValue();
					if (mayorIzq) {
						resultado = true;
						break;
			...
			if (listIzq.size() > listDer.size())
				...
				for (int i = 0; i < listIzq.size(); i++) {
					boolean mayorIzq = ((BooleanType) mayor(listIzq.get(i),
							listDer.get(i))).getBooleanValue();
					boolean mayorDer = ((BooleanType) mayor(listDer.get(i),
							listIzq.get(i))).getBooleanValue();
					if (mayorIzq) {
						resultado = true;
						break;
			...
			if (dictIzq.size() > dictDer.size())
				...
				boolean keyMayorIzq = false;
				boolean keyMayorDer = false;
				for (Type keyIzq : dictIzq.keySet()) {
					for (Type keyDer : dictDer.keySet()) {
						keyMayorIzq = ((BooleanType) mayor(keyIzq, keyDer))
								.getBooleanValue();
						keyMayorDer = ((BooleanType) mayor(keyDer, keyIzq))
								.getBooleanValue();
						if (keyMayorIzq) {
							resultado = true;
							break;
					...
					if ((keyMayorIzq) || (keyMayorDer)) {
						break;
					}
				}
				if ((!keyMayorIzq) && (!keyMayorDer)) {
					boolean valueMayorIzq = false;
					boolean valueMayorDer = false;
					for (Type valueIzq : dictIzq.values()) {
						for (Type valueDer : dictDer.values()) {
							valueMayorIzq = ((BooleanType) mayor(valueIzq,
									valueDer)).getBooleanValue();
							valueMayorDer = ((BooleanType) mayor(valueDer,
									valueIzq)).getBooleanValue();
							if (valueMayorIzq) {
								resultado = true;
								break;
							} else if (valueMayorDer) {
								resultado = false;
								break;
							}
						}
						if ((valueMayorIzq) || (valueMayorDer)) {
							break;
						}
					}
				}
			...
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion mayor");
		}
	...
	private Type menorIgual(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			...
			if (tupleIzq.size() < tupleDer.size())
				...
				for (int i = 0; i < tupleIzq.size(); i++) {
					boolean menorIzq = ((BooleanType) menorIgual(
							tupleIzq.get(i), tupleDer.get(i)))
							.getBooleanValue();
					boolean menorDer = ((BooleanType) menorIgual(
							tupleDer.get(i), tupleIzq.get(i)))
							.getBooleanValue();
					if (menorIzq) {
						resultado = true;
						break;
			...
			if (listIzq.size() < listDer.size())
				...
				for (int i = 0; i < listIzq.size(); i++) {
					boolean menorIzq = ((BooleanType) menorIgual(listIzq.get(i),
							listDer.get(i))).getBooleanValue();
					boolean menorDer = ((BooleanType) menorIgual(listDer.get(i),
							listIzq.get(i))).getBooleanValue();
					if (menorIzq) {
						resultado = true;
						break;
			...
			if (dictIzq.size() < dictDer.size())
				...
				for (Type keyIzq : dictIzq.keySet()) {
					for (Type keyDer : dictDer.keySet()) {
						keyMenorIzq = ((BooleanType) menorIgual(keyIzq, keyDer))
								.getBooleanValue();
						keyMenorDer = ((BooleanType) menorIgual(keyDer, keyIzq))
								.getBooleanValue();
						if (keyMenorIzq) {
							resultado = true;
							break;
				...
				if ((!keyMenorIzq) && (!keyMenorDer)) {
					...
					for (Type valueIzq : dictIzq.values()) {
						for (Type valueDer : dictDer.values()) {
							valueMenorIzq = ((BooleanType) menorIgual(valueIzq,
									valueDer)).getBooleanValue();
							valueMenorDer = ((BooleanType) menorIgual(valueDer,
									valueIzq)).getBooleanValue();
							if (valueMenorIzq) {
								resultado = true;
								break;
			...
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion menor o igual");
		}
	...
	private Type mayorIgual(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			...
			if (tupleIzq.size() > tupleDer.size())
				...
				for (int i = 0; i < tupleIzq.size(); i++) {
					boolean mayorIzq = ((BooleanType) mayorIgual(
							tupleIzq.get(i), tupleDer.get(i)))
							.getBooleanValue();
					boolean mayorDer = ((BooleanType) mayorIgual(
							tupleDer.get(i), tupleIzq.get(i)))
							.getBooleanValue();
					if (mayorIzq) {
						resultado = true;
			...
			if (listIzq.size() > listDer.size())
				...
				for (int i = 0; i < listIzq.size(); i++) {
					boolean mayorIzq = ((BooleanType) mayorIgual(listIzq.get(i),
							listDer.get(i))).getBooleanValue();
					boolean mayorDer = ((BooleanType) mayorIgual(listDer.get(i),
							listIzq.get(i))).getBooleanValue();
					if (mayorIzq) {
						resultado = true;
						break;
			...
			if (dictIzq.size() > dictDer.size())
				...
				for (Type keyIzq : dictIzq.keySet()) {
					for (Type keyDer : dictDer.keySet()) {
						keyMayorIzq = ((BooleanType) mayorIgual(keyIzq, keyDer))
								.getBooleanValue();
						keyMayorDer = ((BooleanType) mayorIgual(keyDer, keyIzq))
								.getBooleanValue();
						if (keyMayorIzq) {
							resultado = true;
							break;
				...
				if ((!keyMayorIzq) && (!keyMayorDer)) {
					...
					for (Type valueIzq : dictIzq.values()) {
						for (Type valueDer : dictDer.values()) {
							valueMayorIzq = ((BooleanType) mayorIgual(valueIzq,
									valueDer)).getBooleanValue();
							valueMayorDer = ((BooleanType) mayorIgual(valueDer,
									valueIzq)).getBooleanValue();
							if (valueMayorIzq) {
								resultado = true;
								break;
			...
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion mayor o igual");
		}
	...
	private Type andBit(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			...
			boolean resultado = ((BooleanType) izq).getValue()
					& ((BooleanType) der).getValue();
			res = new BooleanType(resultado);
		} else {
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion and bitwise");
		}
	...
	private Type orBit(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			...
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion or bitwise");
		}
	...
	private Type xorBit(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			...
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion xor");
		}
	...
	private Type rigthShiftBit(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			int resultado = ((IntegerType) izq).getValue() >> ((IntegerType) der)
					.getValue();
			res = new IntegerType(resultado);
		} else if (intLong) {
			long resultado = ((IntegerType) izq).getValue() >> ((LongType) der)
					.getValue();
			res = new LongType(resultado);
		} else if (intBoolean) {
			int resultado = ((IntegerType) izq).getValue() >> ((BooleanType) der)
					.getEquivalentInt();
			res = new IntegerType(resultado);

		} else if (longInt) {
			long resultado = ((LongType) izq).getValue() >> ((IntegerType) der)
					.getValue();
			res = new LongType(resultado);

		} else if (longLong) {
			long resultado = ((LongType) izq).getValue() >> ((LongType) der)
					.getValue();
			res = new LongType(resultado);

		} else if (longBoolean) {
			long resultado = ((LongType) izq).getValue() >> ((BooleanType) der)
					.getEquivalentInt();
			res = new LongType(resultado);

		} else if (booleanInt) {
			int resultado = ((BooleanType) izq).getEquivalentInt() >> ((IntegerType) der)
					.getValue();
			res = new IntegerType(resultado);

		} else if (booleanLong) {
			long resultado = ((BooleanType) izq).getEquivalentInt() >> ((LongType) der)
					.getValue();
			res = new LongType(resultado);
		} else if (booleanBoolean) {
			int resultado = ((BooleanType) izq).getEquivalentInt() >> ((BooleanType) der)
					.getEquivalentInt();
			res = new IntegerType(resultado);
		} else {
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion desplazamiento derecho");
		}
	...
	private Type leftShiftBit(Type izq, Type der) throws CompilerException {
		...
		if (intInt) {
			...
			throw new CompilerException(lineNumber,
					"Tipos de datos no esperados para la operacion desplazamiento izquierdo");
		}
	...
	public Type evaluate() throws CompilerException {
		...
		if (izq == null || der == null) {
			throw new CompilerException(lineNumber,
					"La funcion evaluada no tiene valor de retorno");
		} else {
			if (op.equals("+")) { // SUMA
				res = suma(izq, der);
